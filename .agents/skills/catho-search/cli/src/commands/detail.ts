...
export async function runDetail(opts: DetailOpts): Promise<number> {
  ...
  try {
    // The SEO slug in the path is cosmetic — Catho resolves the posting from the
    // trailing numeric ID alone, so any placeholder slug works.
    const html = await htmlFetch(`${BASE_URL}/vagas/vaga/${id}`)
    if (!html) {
