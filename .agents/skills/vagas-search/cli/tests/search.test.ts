...
// Live smoke tests — hit the real Vagas.com site. Keep volume low (a handful of
...
describe("live search", () => {
  test("search for 'analista' returns real results", async () => {
  ...
  test("detail on a live result returns a readable description", async () => {
    const searchResult = await runCLI(["search", "-q", "analista", "--limit", "1"]);
