...
describe("CLI flag validation", () => {
  ...
  test("search with --page exits 1 with UNKNOWN_FLAG (feed has no real pagination)", async () => {
    const result = await runCLI(["search", "--query", "flutter", "--page", "2"]);
    expect(result.exitCode).toBe(1);
    const err = JSON.parse(result.stderr);
    expect(err.code).toBe("UNKNOWN_FLAG");
  });
  ...
  test("detail with an id missing the company-slug/job-slug shape exits 1 with BAD_ID", async () => {
    const result = await runCLI(["detail", "not-a-composite-id"]);
    expect(result.exitCode).toBe(1);
    const err = JSON.parse(result.stderr);
