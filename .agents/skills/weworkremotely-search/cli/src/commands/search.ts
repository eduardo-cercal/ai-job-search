...
function renderTable(cards: JobDetail[]): string {
  ...
  // The id is the full job slug (e.g. "lemon-io-senior-react-native-developer-1") and is
  // never truncated here — a cut-off id silently produces a different, wrong id when
  // pasted into `detail`, which is worse than a ragged table column.
