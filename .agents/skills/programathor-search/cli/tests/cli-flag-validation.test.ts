...
describe("CLI flag validation", () => {
  ...
  test("search with --jobage exits 1 with UNKNOWN_FLAG (no posting-date field exists)", async () => {
    const result = await runCLI(["search", "--query", "flutter", "--jobage", "7"]);
    expect(result.exitCode).toBe(1);
    const err = JSON.parse(result.stderr);
    expect(err.code).toBe("UNKNOWN_FLAG");
  });

  test("detail always exits 1 with DETAIL_UNSUPPORTED", async () => {
  ...
  test("search with a non-numeric --page exits 1 with BAD_ARG", async () => {
    const result = await runCLI(["search", "--query", "flutter", "--page", "abc"]);
    expect(result.exitCode).toBe(1);
    const err = JSON.parse(result.stderr);
